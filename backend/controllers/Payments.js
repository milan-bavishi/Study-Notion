...
exports.capturePayment = async (req,res) => {
    ...
    try{
        if(courses.length === 0){
            return res.json({
                success: false,
                message: 'Plese provide vaild course ID',
            });
        };
        ...
        for(const course_id of courses){
            ...
            try{
                ...
                //user alraedy pay for the same course
                const uid = new mongoose.Types.ObjectId(userId);
                if(course.studentsEnrolled.includes(uid)){
                    return res.status(200).json({
                        success: false,
                        messge: 'Student is already enrolled',
                    });
                }
...
exports.verifySignature = async (req,res) => {
    ...
    const enrolleStudent = async (courses,userId) => {
        if(!courses || !userId){
            return res.status(400).json({
                success: false,
                message: 'Plese provide vaild courses and user ID',
            });
        }
        ...
        try{
            ...
            for(const course_id of courses){
                ...
                //set course progress
                const newCourseProgrss = new CourseProgress({
                    userID: userId,
                    courseID: course_id,
                })
                await newCourseProgrss.save();

                //add new course progress to user
                await User.findByIdAndUpdate(userId,{
                    $push: {courseProgress: newCourseProgrss._id},
                },{new: true});
...
exports.sendPaymentSuccessEmail = async (req, res) => {
    ...
    if(!amount || !paymentId){
        return res.status(400).json({
            success: false,
            message: 'Plese provide vaild payment details',
        });
    }
    try{
